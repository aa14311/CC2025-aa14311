...
let buttonY = 100;
let buttonD = 80;
let hovering = false; // boolean variable that tracks
// whether the mouse is hovering over my button
...
function draw() {
  background(r,g,b); // use r g and b variables for bg colors
  ...
  if(distance<buttonD/2){; // is the distance between the mouse and
  // the center of the button LESS THAN the radius?
  fill(50);
  hover = true; // FLIPPED hovering variable
  if(mouseIsPressed){
    strokeWeight(3);
  }
  ...
  circle(ballX,200,30); // circle uses ballx for x position
}

function mousePressed(){ //the mousePressed function runs enclosed
  // code ONCE when teh mouse is pressed down
  if(distance == true){
  r = random(255);
  g = random(255);
  b = random(255);
